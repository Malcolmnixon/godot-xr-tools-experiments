...
var original_mode
var highlight_material_active = null
...
func _update_highlight():
	if highlight_mesh_instance_node:
		...
		for i in range(0, highlight_mesh_instance_node.get_surface_material_count()):
			if closest_count > 0:
				highlight_mesh_instance_node.set_surface_material(i, highlight_material_active)
			else:
...
func _ready():
	# Load default highlight material if not specified
	highlight_material_active = highlight_material;
	if !highlight_material_active:
		highlight_material_active = preload("res://addons/godot-xr-tools/materials/highlight.tres")
